...
Which halves you read, and what you do with them, depends on the path you choose.

**1 — Conceptual read.** About two hours, nothing installed, no cost. Read chapters 0–3, then Part 1 of each stage, then Parts III and IV. You finish knowing what an agent is, how a graph differs from an ordinary script, and why each design decision was made — without writing a line of code.

**2 — Guided build, with an AI coding agent.** The recommended path if you are not a developer. A *coding agent* is an AI assistant that can create files and run commands on your computer, rather than only talking about them; Claude Code is the one this project was built with. Give it this document and work through the stages together: it explains the concept, writes the file, tells you the exact command to run, and waits while you read the output. The questions in Appendix E make good checkpoints between stages. You supply the judgement and the two API keys; it does the typing.

**3 — Build-along, by hand.** The same order, but you write each file yourself from the code in Part 2 of each stage. Slower, and the surest way to internalise the mechanics. Start with chapter 4 to set up your environment.

Whichever you choose, the stages are cumulative. Do not skip ahead: Stage 7 is incomprehensible without Stages 5 and 6, and Stage 9 depends on a subtlety introduced in Stage 7.
